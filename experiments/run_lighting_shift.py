...
def write_header(path: str) -> None:
    header = (
        ...
        "latency_ms,hardware,lighting_level\n"
    )
    ...
    with open(path, "a", encoding="utf-8") as f:
        for cls_name, mean_conf, std_conf, _ in detections:
            f.write(
                ...
                f"{std_conf:.4f},{latency_ms:.2f},{hardware},{lighting_level}\n"
            )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Lighting-shift condition with MC Dropout YOLOv8."
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="reduced",
        help="Qualitative lighting level description to log (e.g., reduced, strong_backlight).",
    )
    ...
    print("Adjust lighting according to the chosen lighting_level while the script runs.")
