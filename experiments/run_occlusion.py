...
def write_header(path: str) -> None:
    header = (
        ...
        "latency_ms,hardware,occlusion_level\n"
    )
    ...
    with open(path, "a", encoding="utf-8") as f:
        for cls_name, mean_conf, std_conf, _ in detections:
            f.write(
                ...
                f"{std_conf:.4f},{latency_ms:.2f},{hardware},{occlusion_level}\n"
            )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Occlusion condition with MC Dropout YOLOv8."
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="50_percent",
        help="Qualitative occlusion level description to log (e.g., 25_percent, 50_percent, 75_percent).",
    )
    ...
    print("Manually introduce occlusions (e.g., covering part of the object) at the specified level while the script runs.")
