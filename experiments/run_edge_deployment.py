    ...
    with open(path, "a", encoding="utf-8") as f:
        for cls_name, mean_conf, std_conf, _ in detections:
            f.write(
                ...
                f"{std_conf:.4f},{latency_ms:.2f},{hardware}\n"
            )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Edge deployment condition with MC Dropout YOLOv8 (intended for Raspberry Pi 5)."
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of MC Dropout samples per frame (lower for edge hardware).",
    )
    ...
    # On edge hardware you may want to swap in a quantized model path here.
    ...
    print(f"[{condition}] Edge experiment started. Logging to {output_path}")
    ...
    try:
        ...
        print(f"[{condition}] Edge experiment finished after {frame_idx} frames.")
